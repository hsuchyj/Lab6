...
public class PokerTableController implements Initializable {
	...
	public void btnDrawCard_Click(ActionEvent event) throws InterruptedException {
		...
		for (int iPosition = 1; iPosition < 3; iPosition++) {
			...
			PathTransition pathT = CreatePathTransition(pntDeck,
			 pntCardDealt, img);			
			PathTransition path2 = CreatePathTransition(pntDeck,
					 pntCardDealt, img);
			...
			patTMoveRot.getChildren().add(pathT);
			part2.getChildren().addAll(path2,rotT);
			...
			seqDeal.getChildren().addAll(patTMoveRot, part2, patTFadeInFadeOut);
	...
	//make changes here for lab6
	private PathTransition CreatePathTransition(Point2D fromPoint, Point2D toPoint, ImageView img) throws InterruptedException {
		...
		path.getElements().add(new LineTo(x+150.0f, y));
		int inc = 0;
		//goes in circle three times, each time gets bigger
		for(int i = 0; i < 3;i++)
		{
			path.getElements().add(new QuadCurveTo(300,10,500+inc, y));
			path.getElements().add(new QuadCurveTo(300,500,x+150.0f-inc, y));
			inc+=100;
		...
		if(counter % 2 != 0)
		{
			path.getElements().add(new LineTo(toPoint.getX(), toPoint.getY()));
